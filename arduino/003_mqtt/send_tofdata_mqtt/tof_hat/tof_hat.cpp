...
uint16_t tof_hat::measure_distance()
{
...
uint16_t tof_hat::bswap(byte b[]) {
  // Big Endian unsigned short to little endian unsigned short
  uint16_t val = ((b[0] << 8) & b[1]);
  return val;
}

uint16_t tof_hat::makeuint16(int lsb, int msb) {
    return ((msb & 0xFF) << 8) | (lsb & 0xFF);
}

uint16_t tof_hat::VL53L0X_decode_vcsel_period(short vcsel_period_reg) {
...
void tof_hat::write_byte_data(byte data) {
...
void tof_hat::write_byte_data_at(byte reg, byte data) {
  Wire.beginTransmission(ToF_ADDR);
  Wire.write(reg);
  Wire.write(data);
  Wire.endTransmission();
...
void tof_hat::write_word_data_at(byte reg, uint16_t data) {
  byte b0 = (data &0xFF);
  byte b1 = ((data >> 8) && 0xFF);
...
byte tof_hat::read_byte_data_at(byte reg) {
...
uint16_t tof_hat::read_word_data_at(byte reg) {
  write_byte_data(reg);
...
void tof_hat::read_block_data_at(byte reg, int sz) {
  int i = 0;
  write_byte_data(reg);
